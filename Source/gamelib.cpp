/*
 * gamelib.cpp: 本檔案儲存支援游戲相關的class的implementation
...
{
	//
	// digit[i]為class varibale，所以必須避免重復LoadBitmap
	//
	if (!isBmpLoaded) {
...
void CGameState::ShowInitProgress(int percent)
{
	...
	CDDraw::BltBackColor(DEFAULT_BG_COLOR);		// 將 Back Plain 涂上預設的顏色
...
/////////////////////////////////////////////////////////////////////////////
// CGame: Game Class
// 這個class是游戲的facade，是MFC與各個游戲狀態的橋梁，如果不增加或減少
// 游戲狀態的話，可以不用管這個class的介面與實作。
/////////////////////////////////////////////////////////////////////////////
...
void CGame::OnDraw()
{
	CDDraw::BltBackColor(DEFAULT_BG_COLOR);	// 將 Back Plain 涂黑
	gameState->OnDraw();					// 顯示游戲中的每個元素
	if (!running) {
		//
		// 如果在暫停狀態，則顯示Ctrl-Q...
		//
...
bool CGame::OnIdle()  // 修改功能不要修改OnIdle()，而應修改OnMove()及OnShow()
{
	...
	//
	// 以下的程式控制游戲進行的速度，注意事項：
	// 1. 用Debug mode可以檢視每一次回圈花掉的時間，令此時間為t。
...
void CGame::OnInit()	// OnInit() 只在程式一開始時執行一次
{
	...
	// 開啟DirectX繪圖介面
	//
	CDDraw::Init(SIZE_X, SIZE_Y);							// 設定游戲解析度
	//
	// 開啟DirectX音效介面
	//
